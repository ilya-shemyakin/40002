...
struct Polygon
{
    ...
    double area() const
    {
        ...
        s+=1LL*points.back().x*points.front().y-1LL*points.front().x*points.back().y;
    ...
    bool isRectangle() const
    {
        ...
        {
            Point v1{points[(i+1)%4].x-points[i].x,points[(i+1)%4].y-points[i].y};
            Point v2{points[(i+3)%4].x-points[i].x,points[(i+3)%4].y-points[i].y};
...
inline bool intersects(const Polygon&a,const Polygon&b)
{
    ...
    if(pointInPoly(a,b.points[0])||pointInPoly(b,a.points[0])) return true;
    return false;
}
inline bool parsePolygon(const std::string& line, Polygon& out)
{
    std::istringstream iss(line);
    std::size_t n; if(!(iss>>n)) return false;
    ...
    if(tmp.points.size()!=n) return false;
    std::swap(out, tmp);
    return true;
}
}
