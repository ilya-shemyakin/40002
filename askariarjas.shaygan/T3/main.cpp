...
int main(int argc, char* argv[]) {
    if (argc != 2) { std::cerr << "Error: Filename required\n"; return 1; }
    ...
    while (std::getline(fin, line)) {
        ...
        Polygon poly;
        if (readPolygon(line, poly)) polygons.push_back(std::move(poly));
    }
    fin.close();
    ...
    while (std::getline(std::cin, line)) {
        ...
        std::string cmd, arg; iss >> cmd;
        if (cmd == "AREA") {
            iss >> arg;
            if (arg == "EVEN" || arg == "ODD") {
                bool wantEven = arg == "EVEN";
                double sum = std::accumulate(polygons.begin(), polygons.end(), 0.0,
                    [wantEven](double acc, Polygon const& p) {
                ...
                double sum = std::accumulate(polygons.begin(), polygons.end(), 0.0,
                    [](double acc, Polygon const& p) { return acc + calculateArea(p); });
            ...
            if (arg == "EVEN" || arg == "ODD") {
                ...
                try {
                    size_t v = std::stoul(arg);
                    int cnt = std::count_if(polygons.begin(), polygons.end(),
                        [v](Polygon const& p) { return p.points.size() == v; });
                    std::cout << cnt << "\n";
                }
                catch (...) {
                    std::cout << "<INVALID COMMAND>\n";
            ...
            Polygon q; q.points.reserve(n);
            bool bad = false;
            for (int i = 0;i < n;i++) {
                std::string tok; if (!(iss >> tok)) { bad = true; break; }
                Point pt; if (!parsePoint(tok, pt)) { bad = true; break; }
                q.points.push_back(pt);
            }
            if (bad) { std::cout << "<INVALID COMMAND>\n"; continue; }
            double a0 = calculateArea(q);
            int cnt = std::count_if(polygons.begin(), polygons.end(),
                [a0](Polygon const& p) { return calculateArea(p) < a0; });
            std::cout << cnt << "\n";
        }
            ...
            Rect r0(q);
            int cnt = std::count_if(polygons.begin(), polygons.end(),
                [r0](Polygon const& p) { return Rect(p).intersects(r0); });
            std::cout << cnt << "\n";
        }
        else {
            std::cout << "<INVALID COMMAND>\n";
